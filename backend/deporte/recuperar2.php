...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("SELECT id_administrador FROM admin_deporte WHERE dni = :dni AND email = :email");
    ...
    if ($afiliado) {
        ...
        // Guardar token
        $update = $pdo->prepare("UPDATE admin_deporte SET token_recuperacion = :token, token_expiracion = :exp WHERE id_administrador = :id");
        ...
        $url_recuperacion = "http://localhost/sindicato/backend/deporte/resetear_clave2.php?token=" . $token;
