...
if (empty($token)) {
    ...
    $stmt = $pdo->prepare("SELECT id_administrador FROM admin_deporte WHERE token_recuperacion = :token AND token_expiracion > NOW()");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['nueva_clave'])) {
    ...
    if ($nueva_clave !== $confirmar_clave) {
        ...
        $update = $pdo->prepare("UPDATE admin_deporte SET contrasena = :pass, token_recuperacion = NULL, token_expiracion = NULL WHERE id_administrador = :id");
        
        if ($update->execute(['pass' => $password_hash, 'id' => $admin['id_administrador']])) {
...
<body>
    <div class="container" style="max-width: 450px; margin-top: 50px;">
        ...
        <div style="text-align:center; margin-top:20px;">
            <a href="login_admin_afil.php" style="color:#666; text-decoration:none;">Ir al Login</a>
        </div>
    </div>
</body>
</html>