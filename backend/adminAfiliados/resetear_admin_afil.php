...
if (empty($token)) {
    ...
    $stmt = $pdo->prepare("SELECT id_administrador FROM administrador WHERE token_recuperacion = :token AND token_expiracion > NOW()");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['nueva_clave'])) {
    ...
    if ($nueva_clave !== $confirmar_clave) {
        ...
        $update = $pdo->prepare("UPDATE administrador SET contrasena = :pass, token_recuperacion = NULL, token_expiracion = NULL WHERE id_administrador = :id");
