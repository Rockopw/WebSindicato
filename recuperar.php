...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Usaremos 'afiliados' y 'afiliados_id' como pusiste en tu primer bloque
    $stmt = $pdo->prepare("SELECT afiliados_id FROM afiliados WHERE dni = :dni AND email = :email");
    ...
    if ($afiliado) {
        ...
        // Guardar token
        $update = $pdo->prepare("UPDATE afiliados SET token_recuperacion = :token, token_expiracion = :exp WHERE afiliados_id = :id");
        $update->execute(['token' => $token, 'exp' => $expiracion, 'id' => $afiliado['afiliados_id']]);

        // URL real (Cambia 'tu-web.com' por 'localhost/nombre_tu_carpeta' si estás en local)
        $url_recuperacion = "http://localhost/sindicato/resetear_clave.php?token=" . $token;
...
<body>
    <div class="container" style="max-width: 450px; margin-top: 50px;">
        ...
        <form method="POST">
            ...
            <a href="login.php" style="display:block; text-align:center; margin-top:15px; color:#666;">Volver al Login</a>
        </form>
    </div>
</body>
</html>